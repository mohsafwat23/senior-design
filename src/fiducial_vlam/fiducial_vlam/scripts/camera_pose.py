...
class ArucoNode(Node):
  ...
  #============================================================================
    ...
    self.pitch_offset = 3.0# * np.pi / 180.0
  ...
  def listener_callback(self, data):
    ...
    if self.distance > self.distance_des:
      ...
      #self.this_aruco_parameters = cv2.aruco.DetectorParameters_create()
      self.aruco_marker_side_length = 0.1778
      #self.aruco_marker_side_length = 0.21336# 0.1355
    ...
    if marker_ids is not None:
      ...
      for i, marker_id in enumerate(marker_ids):  
        ...
        tvecs_old_2 = np.array([-tvecs[i][0][0], tvecs[i][0][1],tvecs[i][0][2]])
        ...
        #print("tvecs", np.dot(rot, np.matrix(-tvecs[i][0]).T))
        print("rotation matrix: ", tvecsnew)
        print("rot act: ",np.dot(rotation_matrix[0:3, 0:3].T, np.matrix(-tvecs[i][0]).T))

        #print("rot act: ",rotation_matrix[0:3, 0:3].T)
        ...
        t.transform.rotation.y = self.quat3[2] 
  ...
  def imu_subscriber(self,msg):
    ...
    self.drone_pitch = (msg.pitch-self.pitch_offset)*np.pi/180 
